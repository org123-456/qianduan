...
function createRenderer(container, opts) {
  ...
  function resize() { W = container.clientWidth || window.innerWidth; H = container.clientHeight || window.innerHeight; camera.aspect = W / H; camera.updateProjectionMatrix(); renderer.setSize(W, H); controls.handleResize(); }
  async function load() {
...
export const MemoryEngine = {
    ...
    // 🌟 初始化 3D 星海
    async initSky() {
        if (this.skyInstance) return; // 避免重复加载
